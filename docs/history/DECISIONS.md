...
Decision:
Два исходных ролика SIGMA используются в двух проекциях: четыре зоны
`рынок → боли → MVP → деньги` и десять проверяемых шагов. Каждый factory-run
ведётся через status board: шаг, работа, evidence, решение, следующий gate.
