...
## DEC-2026-09-09-CONTEXT-FIRST-REFERENCE-MINING — Контекст и готовые сервисы до score

Status: active
Area: methodology | research | UX | data
Decision date: 2026-09-09
Evidence: первый психологический проход пропустил существующее Rule 24
исследование и ошибочно сузил рынок до РПП-подсегментов; прямое уточнение
владельца о цели 100 подписчиков примерно по 1 000 ₽ в месяц
Supersedes: автоматическое добавление сырой quick-scan идеи в active hit parade

Decision:
Перед новым поиском в домене, где владелец называет прежние проекты, документы
или исследования, агент обязан провести `context_inventory`. Он показывает
реально прочитанные источники, существующие активы, исследованные гипотезы,
каналы доступа и gaps. До owner checkpoint не создаются active-card, score и
строки hit parade.

После контекста фабрика строит Jobs map и выполняет `reference_mining`: разбирает
готовые сервисы, их ICP, тарифы, onboarding, ручные альтернативы и независимый
голос клиента. Цель — найти проверяемую дыру в подтверждённой категории, а не
копировать код, бренд или интерфейс. Review mining остаётся E1 и переходит в
конкретную ставку только после owner checkpoint.

Why:
Доступ к узкому сообществу — это канал первых интервью и тестов, но не размер
рынка. Для subscription micro-SaaS важнее частая работа, регулярный результат,
путь к первым 100 плательщикам, churn-risk и цена delivery, чем красивая
подгипотеза для малого круга знакомых.

Do:
Хранить context inventory, Jobs map и reference mining в `data/discovery/`.
Разделять «рынок», «доступ к первым людям», «готовый референс» и «нашу ставку».
Показывать в каждом checkpoint: прочитанное, подтверждённое, неизвестное,
действие агента, действие владельца, критерий готовности и следующий gate.

Do not:
Не превращать названный пользователем канал или ранний AI-synthesis в TAM.
Не добавлять идею в active portfolio до согласования конкретной модели. Не
использовать отзывы как замену прошлому поведению, действию или оплате.
