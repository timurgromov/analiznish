...
Текущий этап: factory v1.1 `active`, но не `validated` полным рыночным проходом.
...
* Первый factory-run перезапущен 2026-09-09 в направлении частных психологов:
  ...
  обсуждения. Этапы 3–4 завершены: Wordstat, локальные альтернативы и data-risk
  сохранены, затем Jobs сравнены без продуктовых баллов. В конкурентный разбор
  прошли только A «работа после сессии» и C «путь от обращения до консультации»;
  общий CRM припаркован. `ACTIVE_RUN` фиксирует один следующий gate: этап 5 —
  собрать по пять референсов, onboarding, цены, обходы и жалобы для A/C; затем
  независимый голос клиента и только потом INSIGHT.
