...
* Есть `docs/IDEA_PURGATORY_PROTOCOL.md` и упорядоченный
  ...
  одну дешёвую проверку относительно B2B-цели `1 000–5 000 ₽ × 100–1 000`
  плательщиков, но не смешивается с доказательным hit parade.
...
* Есть `data/ACTIVE_RUN.md` и `docs/RAIL_PROTOCOL.md`: новый чат продолжает
  ...
  их связность. Текущий run — психологи, `S · SCAN 4 — ранний Portfolio Gate`.
...
* Есть `docs/REFERENCE_MINING_PROTOCOL.md`: разбор готовых сервисов, ручных
  альтернатив, независимых отзывов и фильтра массовой подписки.
...
* Первый factory-run перезапущен 2026-09-09 в направлении частных психологов:
  ...
  приглашение, guide и kill criteria; реальных интервью ещё нет. После аудита
  порядка run возвращён на этап 4: сначала A/B/C сравниваются по 18 критериям,
  ёмкости и пути к 100/1 000 плательщикам. Только 1–2 финалиста допускаются к
  реальным интервью.
...
* 2026-09-10 чистилище идей и ранний Portfolio Gate синхронизированы во всех
  правилах; `./scripts/check-local.sh` и `git diff --check` прошли. Активный
  психологический run находится на `S · SCAN 4`, до интервью.
