...
* Первый factory-run перезапущен 2026-09-09 в направлении частных психологов:
  ...
  обсуждения. Этап 3 завершён: Wordstat проверен вручную, локальные альтернативы
  и ограничения данных сохранены; сильнее всего виден Job поиска клиентов,
  однако это ещё спрос на каналы/агрегаторы, не на SaaS. `ACTIVE_RUN` фиксирует
  один следующий gate: этап 4 — сопоставить три Jobs по боли, повторяемости,
  цене ошибки, WTP, сложности и причине сменить альтернативу; затем конкуренты
  и только после них голос клиента/INSIGHT.
