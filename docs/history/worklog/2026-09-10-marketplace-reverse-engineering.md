...
* После уточнения владельца добавлена явная модель доступа: публичный web,
  авторизованный браузер и письмо/скриншот. Зафиксировано, что публичный API
  Acquire.com Marketplace не подтверждён, а `developer.acquire.io` относится к
  другому сервису.
* Коды marketplace evidence оставлены внутренним контролем; обычный ответ
  должен сначала объяснять простыми словами, что именно открыто и подтверждено.
