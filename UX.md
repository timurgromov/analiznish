# UX — Анализ Ниш

Проект работает как conversational tool плюс локальный read-only dashboard. Главный UX — быстрый и честный ответ в чате, а dashboard нужен, чтобы визуально видеть хит-парад, формулу и критерии.

## Core Modes

### Niche Factory

Цель: начать поиск без готовой идеи и довести множество направлений до одной
проверяемой B2B-ставки.

Триггер: пользователь пишет «Хочу искать нишу».

Первый ответ агента:

1. Коротко объяснить текущий checkpoint.
2. Уточнить B2B/B2C, географию, доступ к отраслям/людям, желаемый чек и sales
   cycle, лимит теста, capabilities и hard exclusions.
3. Если пользователь не знает ответы, предложить разумные defaults и 3–5 hunting-направлений.
4. Согласовать `agent_led`, `guided` или `hybrid`; по умолчанию `hybrid`.

Каждый следующий ответ показывает только текущий checkpoint:

1. `Подтверждено`.
2. `Неизвестно`.
3. `Агент делает сейчас`.
4. `От пользователя нужен один конкретный шаг`.
5. `Критерий готовности`.
6. `Следующий gate`.

Нормальный прогресс: `5–10 кандидатов → 3–5 quick scans → 1–2 deep research →
1 concrete bet → problem interviews → offer action → paid pilot → MVP`.

### Idea Inbox

Цель: быстро выгрузить идею из головы без превращения ее в обязательство.

Формат ответа:

1. Одна строка сути идеи.
2. Предварительный тип: `business`, `content`, `automation`, `asset`, `research`, `personal tool`.
3. Статус: `inbox`, `quick_scan_candidate`, `deep_score_candidate`, `parked`, `discarded`.
4. 1-2 причины, почему статус выбран.
5. Один минимальный вопрос, если без него невозможно понять, что это за идея.

В этом режиме не считать score и не обновлять hit parade. Идея может быть просто зафиксирована, чтобы не занимать внимание.

### Quick Scan

Цель: отсечь или поставить идею в очередь за 5-10 минут.

Формат ответа:

1. Вердикт одной строкой.
2. 3-5 ключевых причин.
3. Предварительные scores с `evidence_confidence <= 0.55`, если источники не проверены.
4. `validation_stage`, strongest evidence E0–E5 и главный риск.
5. Один следующий тест с success/kill criteria.

### Deep Score

Цель: полноценная оценка для хит-парада.

Формат ответа:

1. Входные данные и допущения.
2. Hard filters.
3. Evidence table.
4. Market capacity: TAM/SAM/SOM.
5. Unit economics.
6. Score breakdown.
7. CustDev/experiment evidence и текущий gate.
8. Вердикт и hit parade row.
9. Kill conditions.

### Portfolio Review

Цель: выбрать, чем заниматься сейчас.

Формат ответа:

1. Топ-5 карты рынков по `market_opportunity_score` с `evidence_confidence` рядом.
2. Топ-5 конкретных ставок по `execution_priority_score`.
3. Отдельно сильные рыночные референсы без нашей определённой модели.
4. Топ-5 по `economics_score`.
5. Топ-5 по `moat_scale_score`.
6. Ниши, которые нужно закрыть или поставить на паузу.
7. Рекомендация фокуса на ближайшие 7-14 дней.

## Tone

Ответ должен быть прямым и полезным:

* если идея слабая — говорить прямо;
* если идея может дать быстрые деньги, но плохо масштабируется — разделять эти выводы;
* если идея красивая, но без рынка — не спасать её фантазиями;
* если данных мало — не притворяться точностью.

## Local Dashboard

Первый экран — рабочий инструмент:

* карта рынков и референсов;
* очередь конкретных ставок;
* две независимые формулы вместо одного итогового балла;
* расшифровка `Рынок`, `Экономика`, `Защита и масштаб`, `Личный фильтр`, `Доверие`;
* матрица критериев ниши;
* выбранная строка с главным риском и следующим шагом.

Dashboard в P0 read-only. Источник правды остается в Markdown, чтобы не заводить преждевременную БД и админку.

Не делать лендинг как первый экран: проект внутренний и операционный.
