...
16. До реальных интервью обязателен ранний gate по 18 критериям из
    ...
    Portfolio Gate для batch 5–10 **независимых** идей: 1–2 финалиста идут в
    `S5_COMPETITORS`. Если владелец принёс одну ставку с `Research Unit:
    single_thesis`, это Single-thesis Gate для той же одной карточки; нельзя
    искусственно заполнять batch её сегментами, delivery или ценой. В обоих
    случаях проверяются деньги, ожидаемая прибыль, cashflow и стоимость
    исполнения; после S5 владелец выбирает, открывать ли `I_E1`.
    Если закрываются несколько исторических batch/single-thesis runs, затем
    проводится единый Global Portfolio Gate: только при пустых очередях он
    оставляет максимум три предварительных финалиста, а владелец выбирает из
    них ровно один. Выход из текущего соревнования — `parked` с условием
    возврата, не `failed`.
