...
2. Если ты принёс конкретную идею, агент сначала задаёт до четырёх вопросов за
   раз, возвращает один Thesis Contract и получает подтверждение смысла. До
   этого он не исследует подменённую формулировку и не создаёт варианты по
   сегментам или delivery.
3. Если готовой идеи нет, агент сначала покажет, какие твои материалы и активы
   он прочитал, что уже известно и чего не хватает. До подтверждения этой карты
   он не создаёт новых ставок в рейтинге.
4. Если готовой идеи нет, агент ищет кандидатов по трём входам: pain-first
...
5. До интервью обязателен ранний Gate: для поиска с нуля это Portfolio Gate по
   независимым идеям; для одной идеи — Single-thesis Gate без размножения
   упаковок. Opportunity, readiness и confidence не смешиваются.
6. Если объединяются несколько завершённых runs, общий gate при пустых
   исследовательских очередях показывает весь пул ставок, прошедших S0–S5.
   Владелец выбирает пакет подготовки из 1–3 проектов и назначает одну текущую
   работу; остальные desk-квалифицированные ставки не считаются проигравшими.
7. Проекты рабочего пакета последовательно проходят E1-подготовку, реальный
   CustDev, тест оффера и paid-pilot gate; одновременно активна одна работа.
8. Только после build gate допускается полноценный MVP.
9. Карточки, эксперименты, рейтинг и project memory обновляются по ходу цикла.
...
Неготовность новой идеи — нормальная стартовая точка. Отсутствие готового
канала, продукта, кейсов, moat или доказанных CAC/LTV/repeat превращается в
следующий эксперимент, а не в автоматический отказ. Score сортирует работу,
но не закрывает ставку без hard blocker или явного owner decision.
...
| `docs/IDEA_DEFINITION_PROTOCOL.md` | Короткое owner interview, Thesis Contract и защита одной идеи от смыслового дрейфа |
...
`prompts/run-concrete-idea.md` для одной идеи без подмены смысла,
...
На экране решений четыре итоговые группы кликабельны. «Прошли desk research»
показывает ранжированный пул ставок с завершёнными S0–S5; осторожный рейтинг,
база, доверие и происхождение оценки показаны отдельно. Это ещё не
`interview_ready`: сначала владелец формирует пакет подготовки 1–3 и агент
закрывает I_E1 для текущей ставки. «Нужно уточнить ставку»
показывает только дубли и слишком широкие Thesis Contract; hard blockers
остаются отдельным исходом.
