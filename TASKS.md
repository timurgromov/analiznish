...
Доказать Niche Factory v1.2 как единое чистилище и воронку идей: заземлять новые
идеи, ранжировать их до интервью по 18 критериям и довести одного финалиста до
массовой B2B-подписки. Целевой ориентир — `1 000–5 000 ₽/мес`, `100–1 000`
плательщиков и проверка без преждевременной разработки.
...
- [ ] Закрыть owner checkpoint для психологов.
  - Acceptance: владелец подтверждает разделение Rule24, сессионного ассистента
    и РПП-канала; внешние прежние материалы добавлены по конкретному пути/ссылке
    либо явно исключены. Не считать, что список девяти идей находится в Rule24.
...
- [x] Встроить чистилище идей и ранний Portfolio Gate до интервью.
  - Выполнено 2026-09-10: inbox получил классы `A/B/C/X`; целевая подписная
    модель и ранний 18-критериальный отсев закреплены в workflow.
...
- [ ] Пройти ранний Portfolio Gate для психологов до интервью.
  - Acceptance: A «после сессии», B «ведение практики» и C «входящие обращения»
    сравнены по 18 критериям; видны приблизительная ёмкость, путь к 100/1 000
    плательщикам, подписная причина, hard filters, confidence и 1–2 финалиста.
- [ ] Для следующего поиска собрать верхний batch 5–10 идей из чистилища.
  - Acceptance: кандидаты выведены из Jobs, референсов и контекста; публичный
    `S · SCAN 0–3` выполнен batch-режимом до раннего Portfolio Gate.
