...
Завершить первый end-to-end проход Niche Factory v2. Desk research и единый
Global Portfolio Gate уже закрыты: 28 объектов классифицированы, три ставки
оставлены для выбора. Текущая задача — получить явный owner choice одной
ставки, провести минимум пять реальных интервью, затем пройти action, money,
bounded build и repeat gates либо принять честный pivot/park/kill. Повторяемый
...
- [x] Свести психологический batch в единый Global Portfolio Gate.
  - Выполнено 2026-09-13: `psych-lead-to-paid-session` вошёл в итоговую тройку;
    остальные психологические ставки получили условия возврата. Отдельный
    P1/P2 owner gate больше не управляет общей очередью.
...
- [x] Ввести обязательный Definition Gate для конкретной идеи и отделить
  opportunity от readiness.
  - Выполнено 2026-09-13: добавлены owner interview и Thesis Contract; одна идея
    не дробится на сегментные/delivery-проекты; score не назначает outcome;
    отсутствие готового канала, продукта, кейсов, moat и доказанных CAC/LTV/repeat
    является планом исследования, а не ранним отказом. Правила закреплены в
    schema, validator и contract tests.
- [x] Завершить S0–S5 для восстановленного Codex Project Starter.
  - Выполнено 2026-09-13: проверены платные модели, problem signals, локальные
    плательщики, bounded offer и конкуренты. Это одна ставка, а не набор
    продуктов; она вошла в итоговую тройку с E1-неизвестными.
...
- [x] Завершить bounded screen B2B operations batch.
  - Выполнено 2026-09-13: дебиторка и compliance прошли S0–S5; остальные ставки
    остановлены на первом честном неизвестном с измеримым условием возврата.
- [x] Закрыть Portfolio Closure существующих и новых ставок.
  - Выполнено 2026-09-13: 3 предварительных финалиста, 20 вне текущего
    соревнования, 2 hard failed exact-модели и 3 reference/benchmark. Пустых
    `active_research` и `queued_research` нет.
- [ ] Собрать новый S0 batch только при пустом Portfolio Closure.
  - Acceptance: owner не оставил surviving finalist без решения либо выбранный
    ...
    наблюдаемый локальный сегмент и пересечение минимум двух независимых входов
    до записи в реестр.
...
- [ ] Выбрать одну `concrete_bet` для `I_E1`.
  - Текущий выбор: контроль ранней дебиторки малых B2B-услуг, Codex Project
    Starter или обращение → оплаченная первая сессия.
  - Acceptance: owner decision, ровно один `selected_for_interviews`, один
    сегмент, плательщик, JTBD, самый рискованный тезис, лимит цикла и kill
    conditions записаны в карточке.
...
Канонический список всех 20 условий возврата находится в
`data/PORTFOLIO_CLOSURE.md`; dashboard читает те же `nextGate` из реестра.
Ключевые примеры: Радарыч — 5 WTP-сигналов одного frequent-route сегмента;
КАДРА — 5/30 платных intent или новый доступный канал; Sansara — 5 локальных
payment intents; Rule24 — 3 switching cases; compliance-календарь — 3
metadata-аудита портфеля; order-to-cash — доступный оптовик, готовый платно
проверить bounded workflow без портала.

OnSud и Авто SEO/CPA остаются рыночными референсами, а не ставками текущего
соревнования. ProfiWatcher hard failed до письменного permission/API или
независимого lawful source.
