...
Завершить первый end-to-end проход Niche Factory v2. Desk research закрыт: 20
ставок вошли в ранжированный desk-квалифицированный пул, 3 требуют уточнить Thesis Contract, 2 имеют hard
blocker, 3 являются benchmark/reference. Текущая задача — сформировать рабочий
пакет 1–3 и назначить одну текущую ставку, провести минимум пять реальных
интервью, затем пройти action, money,
...
- [x] Перестроить dashboard v3 в единый кабинет всех идей.
  ...
  - Исправлено 2026-09-13: все четыре итоговые карточки открывают состав группы;
    верхний экран показывает `20 прошли desk research / 3 уточнить ставку / 2 hard
    blocker / 3 reference`. Внутри двадцатки показан рейтинг `63 → 47`, база,
    доверие и происхождение оценки; desk-квалификация отделена от пакета
    подготовки 1–3, одной текущей работы и фактического `interview_ready` после I_E1.
...
- [x] Пересмотреть Portfolio Closure без искусственной квоты финалистов.
  - Выполнено 2026-09-13: 20 ставок прошли S0–S5 и образуют desk-квалифицированный пул; 3 требуют
    уточнить границу продукта; 2 hard failed exact-модели и 3
    reference/benchmark. Пустых `active_research` и `queued_research` нет.
- [ ] Собрать новый S0 batch только при пустом Portfolio Closure.
  - Acceptance: owner не оставил surviving finalist без решения либо выбранный
    ...
    до записи в реестр.
- [ ] Сформировать пакет подготовки интервью из 1–3 desk-квалифицированных проектов.
  - Acceptance: owner decision, 1–3 `selected_for_interviews`, ровно один
    текущий focus, а для него записаны сегмент, плательщик, JTBD, самый
    рискованный тезис, лимит цикла и kill conditions.
...
- [ ] После `S4_OWNER` подготовить E1-корпус текущего проекта из пакета.
